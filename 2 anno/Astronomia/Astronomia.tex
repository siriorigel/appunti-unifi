...
\begin{itemize}
    \item Unità astronomica: $1 AU = 1.489 \cdot 10^{11}$ metri, equivalente
    ...
    \begin{gather*}
        1 \ pc = 206265 AU = 3.086 \cdot 10^{16} \ m = 3.26 \text{ anni luce}
    \end{gather*}
...
\begin{itemize}
    \item \textbf{Azimut}: coordinate rispetto all'asse Sud-Nord: descrive
    differenza angolare rispetto al Nord (verso Ovest) ($0 \sim 360$°)
    \item \textbf{Altezza}: coordinata rispetto all'orizzonte ($-90$°$\sim 90$°).
\end{itemize}
...
sono sulla Terra):
\begin{itemize}
    \item \textbf{Declinazione} $\delta$: Ossia l'angolo tra l'oggetto e l'equatore celeste che diventa il
...
è determinata in modo univoco da due angoli $\phi$, misurato in senso antiorario 
nel piano $xy$, e $\theta$, che esprime la distanza angolare dal piano $xy$. In modo 
analogo posso definire gli angoli $\phi'$ e $\theta'$ per esprimere la posizione del punto $P$
...
un'ombra su di una \textbf{meridiana}, la quale, al mezzogiorno, individua esattamente
...
$P$ il tempo assoluto di un anno, con $P = 365.2564$ giorni, e il giorno normale come $\tau = 1$ e
...
anni invece che 100.
È possibile sitamare il tempo siderale conoscendo i giorni che mancano all'equinozio primaverile
...
\begin{itemize}
    ...
    \item \textbf{Rapporto focale} ($\frac{f}{\#}= \frac{f}{D}$): quantità adimensionale che descrive 
...
dunque grandezza fisica Wm$^{-2}$Hz$^{-1}$. Il flusso ad una certa frequenza $\nu$ è esprimibile come:
\begin{align}
...
dalla stella. Si può adesso quantificare il flusso di questa stella $f_\lambda$ ad una certa
...
indipendente dalla distanza e corrisponde all'intensità del Sole. La densità di glusso 
...
\begin{gather*}
    m_e \frac{v_n^{2}}{r_n} = \frac{1}{4\pi\epsilon_0} \frac{e^{2}}{r_n^{2}}
\end{gather*}
Si ottiene allora 
\begin{gather*}
...
L'effetto Doppler, nel caso delle onde elettromagnetiche, si verificano le 
seguenti condizioni:
\begin{itemize}
...
la assorbe e la riemette completamente. Un corpo nero è, dunque, una sorta di radiatore ideale
...
$\sigma = \pi A = 5.67 \cdot 10^{-8}$ Wm$^{-2}$ ?$^{-4}$, che prende il nome di \textbf{costante di Stefan-Boltzmann}.
\begin{figure}[h]
    \centering
...
La \textbf{temperatura cinetica} $T_k$ è legata alla velocità media delle molecole nel gas. In particolare, nel caso di un gas ideale, la teoria cinetica dei gas consente di derivare una relazione
...
la velocità $\overline{v}$, ossia la media dei quadrati della somma di tutte 
le velocità, mentre $k$ è la costante di Boltzmann. Dunque
\begin{align}
    T_k = \frac{mv^{2}}{3k}
\end{align} 
...
la magnitudine assoluta delle stelle gli spettri delle stelle. Le stelle nel diagramma non sono distribuite in modo uniforme
...
binarie visuali è determinarne l'orbita in quanto l'orbita del sistema è 
...
Allora 
\begin{gather*}
    v_1 = \frac{2\pi}{P} \sin i \frac{am_2}{m_1 + m_2} \qquad a^{3} = \frac{G(m_1 + m_2)P^{2}}{4\pi^{2}}
\end{gather*}
Per la terza legge di Keplero si arriva alla \textbf{funzione di massa}:
\begin{align}
...
\end{gather*}
Allora le masse vere sono note a meno dell'inclinazione. Le orbite reali 
sono dunque delle ellissi e le relazioni sopra non sono utilizzabili anche
si potrebbe ottenere come parametro \textbf{l'eccentricità}, ossia 
un parametro che permette di determinare quanto circolare è un orbita
...
e sono dunque le uniche stelle binarie di cui si è in grado di misurare 
l'inclinazione e dunque misurare con una buona precisione la massa. Si misura la 
