...
\end{gather*}
perchè in laboratorio ciò che si misura è
\begin{gather*}
    \frac{m_1}{m_2-m_3} = \frac{m_x}{m_a} \frac{V_x}{V_x} = \frac{\frac{m_x}{V_x}}{\frac{m_a}{V_x}} = \frac{\delta_{\text{ass} \ x}}{\delta_{\text{ass} \ a}}
\end{gather*}
Che è proprio la densità relativa ma dell'acqua a temperatura di laboratorio.\\
...
\begin{gather*}
    P - S = m_x g - m_a g \\
    P - S = m_x g - \delta_a V_x g = g(\ \underbrace{m_x- \delta_a V_x}_{m}\ )\\
    m = \frac{P- S}{g} = m_x - \delta_a V_x = m_x - \delta_a \frac{m_x}{\delta_x}=  m_x \left(1 - \frac{\delta_a}{\delta_x}\right) \\
    m_x = \frac{m}{1 - \frac{\delta_a}{\delta_x}} \underset{\text{\fbox{*}}}{\approx} m\left(1 + \frac{\delta_a}{\delta_x}\right)\\
    \text{\fbox{*} Sviluppo di Taylor: } (1-x)^\alpha = 1-\alpha x + ... 
\end{gather*}
Con $P$ che è la forza peso e $S$ è la spinta di Archimede, il pedice $_x$ indica massa e volume veri (si ricorda che $V_x = V_a$), $\delta_a$ che è la denistà dell'aria e $m$ che è la massa misurata.\\
Divido da entrame le parti per il volume e ottengo: 
\begin{align}
    \delta \approx \delta_m \left(1 + \frac{\delta_a}{\delta_x}\right) = \delta_m f_{arc}
\end{align}
Anche qui per trovare la densità dell'aria mi rifaccio ai tabulati delle densità note.\\\\
Si ricorda che:
\begin{gather*}
    \delta_x = \frac{m_x}{V_x}\\
    \delta_x V_x = m_x\\
    S = m_a g \overset{\text{togliendo } g}{\Longrightarrow } \sigma_x = m_a = \delta_a V_x
\end{gather*}
Mentre all'interno del picnometro invece come cambia? \\
...
$m_p$ la massa del picnometro pieno solo di acqua con la sua spinta di Archimede relativa $\sigma_p$:
\begin{gather*}
    m_a = \text{ massa dell'acqua del volume dell'oggetto}\\
    m_p = \text{ massa del picnometro pieno (solamente) di acqua}\\
    m_1,m_2,m_3 \qquad \text{stessi della proced. 1}
\end{gather*}
\begin{gather*}
    m_1 = m_x - \sigma_x \\
    m_2 = (m_x - \sigma_x) + (m_p - \sigma_p) \\
    m_3 = (m_x + m_p - m_a) - \sigma_p
\end{gather*}
...
\begin{gather*}
    ...
    m_2 - m_3 = -\sigma_x + m_a \underset{\text{\fbox{*}}}{=} m_a\left(1 - \frac{\delta_a}{\delta_A}\right) \Rightarrow m_a = \frac{m_2 - m_3}{1 - \frac{\delta_a}{\delta_A}}\\
    \text{\fbox{*}: } m_a - \sigma_x = m_a -m_a (\frac{\delta_a}{\delta_A}) = m_a -\cancel{m_a}(\frac{\delta_a}{\frac{\cancel{m_a}}{V_x}}) = m_a - (\delta_aV_x) = m_a - \sigma_x
\end{gather*}
Con $\delta_A$ che è la densità dell'acqua.
...
\begin{gather*}
    ...
    \approx \delta_{r_m} \left(1 - \frac{\delta_a}{\delta_A}\right) \left(1 + \frac{\delta_a}{\delta_x}\right) \underset{\text{\fbox{*}}}{=} \delta_{r_m} \left(1 - \frac{\delta_a}{\delta_A} + \frac{\delta_a}{\delta_x} + \cancel{\frac{\delta_a\delta_a}{\delta_A\delta_x}}\right)\\
    \text{\fbox{*} l'ultimo termine si toglie perchè: }\frac{\delta_a\delta_a}{\delta_A\delta_x} = \frac{\frac{m_A^2}{V_x}}{\frac{m_a}{V_x}\frac{m_x}{V_x}} = \frac{m_A^2}{\cancel{V_x^2}}\frac{\cancel{V_x^2}}{m_a m_x}
\end{gather*}
Dove $m_A$ è la massa dell'aria e quindi trascurabile.\\
...
\begin{wrapfigure}{r}{0.4\textwidth}
    ...
    \begin{tikzpicture}
        ...
        \draw(0.6,1.4) ..controls(0.67,1.7).. (0.8, 2);
        \draw(0,0) -- (0.6,1.4);
