...
\end{bmatrix}$ es $\times$ var. cost.: $\quad \begin{matrix}
    ...
    \begin{cases}
            ...
            \end{cases}\\
\end{matrix}$\\
\textbf{EDO II° ord. var. costanti} Da un'eq. del tipo: $a y''(x) + b y'(x) + c y(x) = f(x) \qquad \text{con a,b e c numeri reali}$ si cerca la sol. omogenea $y_0(x)$ (col metodo prec.) e una sol. particolare $y_p(x)$ della forma $c_1(x) \textcolor{blue}{y_1(x)} + c_2(x) \textcolor{red}{y_2(x)}$\\
...
\end{pmatrix}$ e $\parallel\vec{N}\parallel = \sqrt{1+\parallel\nabla f \parallel^2}$\\
    ...
    \end{multicols}    
    \noindent
    \textbf{Criteri di convergenza per serie numeriche e serie note}\\
    \begin{multicols}{4}
        \noindent
    Serie geometriche:\\
    ...
    \begin{multicols}{4}
        ...
        $\sum_{n = 1}^{\infty }(-1)^{n}(a_n)$ converge\\
        \columnbreak\\
